...
if( !empty($options) ){	add_theme_support('post-formats', $output); }
...
function ganesh_register_nav_menu(){
	register_nav_menu('primary', 'Primary Navigation Menu');
}
add_action('after_setup_theme','ganesh_register_nav_menu');

function ganesh_posted_meta(){
...
function ganesh_get_attachment($num = 1){
		$output = '';
...
function theme_slug_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'ganesh-sidebar' ),
        'id' => 'ganesh-sidebar',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'ganesh-sidebar' ),
...
function ganesh_get_embed_media($type=array()){
	//the_content(); 
...
function ganesh_get_bs_slides($attachments){
 $output = array();
